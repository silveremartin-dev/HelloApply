 ...
 * VERSION: 6.4.0 (Decoupled Modular Architecture Edition)
 * LAST UPDATED: 24/09/2026 15:15
 * 
 * New in v6.4.0:
 * - Decoupled Candidate Profile: Fully extracted all personal candidate information, portfolio notes, tone references, and geographic preferences into a dedicated 'Profile.gs' configuration file.
 * - Universal Codebase: 'Code.gs' now acts strictly as an agnostic orchestration and rendering engine.
 * 
 * New in v6.3.1:
...
// Note: Candidate identity, templates, geographic mobility, and portfolio notes are defined in Profile.gs
...
function analyzeAndTailor(context, masterCV, cvTemplateText, letterTemplateText, originalUrl) {
  ...
  const dateFr = `${CANDIDATE_PROFILE.city}, le ${now.getDate()} ${monthsFr[now.getMonth()]} ${now.getFullYear()}`;
  const dateEn = `${CANDIDATE_PROFILE.city}, ${monthsEn[now.getMonth()]} ${now.getDate()}, ${now.getFullYear()}`;
  ...
  const prompt = `
    ...
    - 'location': e.g., "${CANDIDATE_PROFILE.city}", "Paris", or "?" if you cannot extract the information.
    - 'is_in_morbihan': true or false. Determine if the office/location of the job is situated within the candidate's home region/department (${CANDIDATE_PROFILE.department || "56"}) in France.
    ...
    1.5. EXECUTIVE WRITING STYLE (Emulate the Gold Standard tone):
       - Emulate the high-impact, prestigious tone of the reference CV below.
...
function createDraft(job, attachments) {
  ...
  const htmlBody = `
    <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.6; color: #333; max-width: 600px; border: 1px solid #e2e8f0; padding: 25px; border-radius: 12px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
      <p style="font-size: 1.1em; margin-top: 0;">Bonjour ${CANDIDATE_PROFILE.firstName || "Silvère"},</p>
      ...
      <p>Les fichiers PDF adaptés (CV${CANDIDATE_PROFILE.includeTechnicalNote ? " enrichi de la Note de réalisation technique" : ""}, Lettre de motivation, et Mémo d'architecture) sont déjà joints à ce brouillon.</p>
...
/**
 * Parses and verifies if the salary is below the minimum annual threshold.
 */
