...
function analyzeAndTailor(context, masterCV, cvTemplateText, letterTemplateText, originalUrl) {
  const prompt = `
    TASK: Adapt the full CV of Silvère Martin-Michiellot ("SilvereMartinMichiellot-CV-full") and tailor it completely to the target job description. You must also tailor the Cover Letter.
    Both customized documents must be produced with high visual fidelity by replacing the original section contents of the templates in Drive.
    ...
    ========================================================================
    CV TEMPLATE ORIGINAL SUBSTRINGS (MAP OF FIND TARGETS):
    You must find each of the literal substrings listed below inside the CV template and replace them with fully tailored, prestigious equivalents.
    
    1. HEADER TITLE:
       "Architecte de Solutions IA Agentielles | Responsable de Projets IT"
       
    2. SUMMARY PROFILE (Replace the entire paragraph below):
       "Architecte et Manager avec plus de 30 ans d'expertise dans le pilotage de systèmes d'information complexes. Architecte spécialisé en systèmes d'IA agentielles, alliant une double expertise en neurosciences cognitives et en ingénierie Java haute performance. Expert dans la conception d'architectures distribuées et la mise en œuvre de solutions d'IA innovantes pour répondre aux défis métier complexes. Expert en refonte de legacy critique et création de solutions distribuées haute performance."
       
    3. CORE SKILLS (Replace each of the 5 categories below):
       - "Direction & Stratégie : Pilotage de budgets, Schéma Directeur (SDSI), AMOA, management de transition (jusqu'à 8 personnes), augmentation de CA (+400%)."
       - "Architecture IA Agentielle : Orchestration de systèmes multi-agents, réseaux de neurones (Deep Learning), automatisation de processus (RPA) et agents conversationnels."
       - "Ingénierie Logicielle : Expert Java (J2SE 1.0 à 25+), Python, C#, Micro-services, Design Patterns, calcul scientifique distribué."
       - "Data Science & Simulation : Deep Learning (CNN, RNN), algorithmes de décision, simulation de systèmes complexes, moteurs 3D (Three.js)."
       - "Infrastructure & DevOps : Cloud (AWS/GCP), Docker, Kubernetes, cybersécurité (encryption, RGPD), administration systèmes (Unix/Windows)."
       
    4. EXPERIENCE BULLETS (Replace each of these 9 bullets with ultra-rich, strategic, metric-heavy equivalents):
       - Lead Architect (GitHub Sponsorship):
       ...
       - Support Informatique Senior (Techteam / Syleps):
       ...
       - Responsable IT / Chef de Projet AMOA (Equitive / Deloitte):
    ...
    5. TECHNICAL ENVIRONMENT (Replace each of the 3 lines below):
       - "Langages : Java (Expert), Python, C#, Javascript, SQL, C++, LISP, PHP, Powershell."
       - "Outils IA : Google Antigravity, TensorFlow, Keras, Gemini 3, ChatGPT, HuggingFace, N8N, MCP"
    ...
    4. CRITICAL SELF-CORRECTION & REVIEW LOOP:
